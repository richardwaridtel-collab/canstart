...
export default function LandingPage() {
  return (
    <div className="bg-white">
      ...
      {/* ── HERO ─────────────────────────────────────────────── */}
      <section className="relative bg-gradient-to-br from-red-700 via-red-600 to-red-800 text-white overflow-hidden min-h-[92vh] flex items-center">
        <div className="absolute inset-0 bg-[url('/maple-pattern.svg')] opacity-5" />

        {/* Floating resume score card — decorative */}
        <div className="hidden lg:block absolute right-12 top-1/2 -translate-y-1/2 w-72 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-5 shadow-2xl">
          <p className="text-xs font-semibold uppercase tracking-widest text-white/60 mb-3">Your Resume Score</p>
          <div className="flex items-end gap-2 mb-3">
            <span className="text-6xl font-black text-green-300">87</span>
            <span className="text-2xl text-white/40 font-semibold mb-1">%</span>
          ...
          <div className="mt-4 pt-4 border-t border-white/10">
            <p className="text-xs text-white/40">Before: 58% match · After tailoring: 87%</p>
          </div>
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full text-sm font-medium mb-6 border border-white/20">
              ...
              Canada&apos;s only AI-powered job platform for newcomers
            </div>
      ...
      <section className="bg-gray-950 py-20 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="text-red-400 text-sm font-semibold uppercase tracking-widest mb-3">The honest comparison</p>
            <h2 className="text-3xl sm:text-4xl font-black text-white">CanStart vs Every Other Job Board</h2>
            <p className="text-gray-400 mt-3 text-lg">Other platforms were built for the general market. CanStart was built for you.</p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left py-4 pr-6 text-gray-400 font-medium w-1/2">Feature</th>
                  <th className="py-4 px-4 text-center">
                    <div className="inline-flex flex-col items-center gap-1">
                      <span className="bg-red-600 text-white font-bold px-3 py-1 rounded-lg text-sm">CanStart</span>
                    </div>
                  </th>
                  <th className="py-4 px-4 text-center text-gray-500 font-medium">LinkedIn</th>
                  <th className="py-4 px-4 text-center text-gray-500 font-medium">Indeed</th>
                  <th className="py-4 px-4 text-center text-gray-500 font-medium">Job Bank</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { feature: 'Built specifically for Canadian newcomers', canstart: true, linkedin: false, indeed: false, jobbank: false },
                  { feature: 'AI resume tailoring to each job description', canstart: true, linkedin: false, indeed: false, jobbank: false },
                  { feature: 'See your match score before applying', canstart: true, linkedin: false, indeed: false, jobbank: false },
                  { feature: 'Verified scam-free employers', canstart: true, linkedin: 'partial', indeed: false, jobbank: 'partial' },
                  { feature: 'Canadian experience building (volunteer & internships)', canstart: true, linkedin: false, indeed: false, jobbank: false },
                  { feature: '100% free for job seekers — no premium tier', canstart: true, linkedin: 'partial', indeed: true, jobbank: true },
                ].map((row, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    <td className="py-4 pr-6 text-gray-300">{row.feature}</td>
                    {[row.canstart, row.linkedin, row.indeed, row.jobbank].map((val, j) => (
                      <td key={j} className="py-4 px-4 text-center">
                        {val === true    && <CheckCircle size={18} className={`mx-auto ${j === 0 ? 'text-green-400' : 'text-green-600'}`} />}
                        {val === false   && <X size={18} className="mx-auto text-gray-600" />}
                        {val === 'partial' && <span className="text-yellow-500 text-xs font-semibold">Partial</span>}
                      </td>
                    ))}
                  </tr>
                ))}
          ...
          <div className="space-y-24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-gray-500 text-lg leading-relaxed mb-6">
                  ...
                  employers are scanning for, without inventing anything. Every certification, degree, and skill you have
                  is preserved and highlighted.
                </p>
                <div className="space-y-3 mb-8">
                  {[
                    'See your current resume score first — then decide to tailor',
              ...
              <div className="bg-gray-50 rounded-2xl border border-gray-200 p-6 shadow-sm">
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-2">
                  ...
                  {['ATS keywords aligned to job posting', 'MBA & certifications preserved', '3 implied skills surfaced honestly', 'Canadian resume format applied'].map((item) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="order-2 lg:order-1 bg-gray-950 rounded-2xl p-6 shadow-sm">
                ...
                <div className="mt-5 pt-4 border-t border-white/10">
                  <p className="text-xs text-gray-500 mb-2">Gaps to close before applying:</p>
                  <div className="flex items-start gap-2 text-xs text-orange-400">
                    <span className="mt-0.5">⚠</span>
                    <span>No Workday or SAP HR experience mentioned</span>
                  </div>
                </div>
              </div>
              <div className="order-1 lg:order-2">
                ...
                <h3 className="text-2xl sm:text-3xl font-black text-gray-900 mb-4">
                  Know your chances before you spend an hour on an application
                </h3>
                <p className="text-gray-500 text-lg leading-relaxed mb-6">
                  Every job on CanStart shows you a live match score against your profile. See which skills are missing,
                  which keywords to add, and exactly what a hiring manager&apos;s ATS will flag — before you click Apply.
                </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-gray-500 text-lg leading-relaxed mb-6">
                  Job scams target newcomers. On CanStart, every business is verified before posting. You&apos;ll never
                  see fake postings, unpaid work disguised as internships, or companies that vanish after you apply.
                </p>
                <div className="space-y-3 mb-8">
                  {[
                    'Business identity verified before going live',
                    'Real-name employer profiles with review history',
                    'Volunteer, micro-internship, and paid roles clearly labelled',
                    'Report a concern — reviewed within 24 hours',
                  ].map((pt) => (
                    <div key={pt} className="flex items-start gap-3 text-sm text-gray-700">
                      <CheckCircle size={16} className="text-green-500 flex-shrink-0 mt-0.5" />{pt}
                    </div>
              ...
              {/* Visual mock */}
              <div className="bg-gray-50 rounded-2xl border border-gray-200 p-6 shadow-sm space-y-3">
                {[
                  { name: 'Maple Tech Solutions', city: 'Ottawa, ON', type: 'Micro-Internship', badge: 'Verified' },
                  { name: 'Northern Data Co.', city: 'Toronto, ON', type: 'Paid Position', badge: 'Verified' },
                  { name: 'Pacific Community Hub', city: 'Vancouver, BC', type: 'Volunteer', badge: 'Verified' },
                ].map((emp) => (
                  <div key={emp.name} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 flex items-center gap-1"><MapPin size={10} />{emp.city}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className={`text-xs font-semibold px-2 py-1 rounded-full ${
                        emp.type === 'Paid Position' ? 'bg-purple-100 text-purple-700' :
                        emp.type === 'Micro-Internship' ? 'bg-blue-100 text-blue-700' :
                        'bg-green-100 text-green-700'
                      }`}>{emp.type}</span>
                      <div className="flex items-center gap-1 justify-end mt-1">
                        <Shield size={10} className="text-green-500" />
                        <span className="text-xs text-green-600 font-medium">{emp.badge}</span>
                      </div>
                    </div>
                  </div>
                ))}
                <p className="text-xs text-gray-400 text-center pt-1">Every posting manually reviewed before going live</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div>
              ...
              <div className="space-y-4">
                {[
                  { step: '1', title: 'Create your free profile', desc: 'Upload your resume, add your skills and immigration status. Takes 3 minutes.', icon: <Users size={16} /> },
                  { step: '2', title: 'Browse & see your match score', desc: 'Browse live Canadian jobs. See your match score on every single one before applying.', icon: <BarChart2 size={16} /> },
                  { step: '3', title: 'Tailor your resume in seconds', desc: 'Paste the job description — AI rewrites your resume for that role. See before vs after score.', icon: <Sparkles size={16} /> },
                  { step: '4', title: 'Apply & track everything', desc: 'Submit your application and track every status in one dashboard.', icon: <Briefcase size={16} /> },
                ].map((item) => (
                  <div key={item.step} className="flex gap-4 bg-white p-5 rounded-xl border border-gray-100 hover:border-red-200 hover:shadow-sm transition-all">
                    <div className="w-9 h-9 bg-red-600 text-white rounded-full flex items-center justify-center font-black text-sm flex-shrink-0">
                      {item.step}
                    </div>
                    <div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {[
                  { step: '1', title: 'Register your business', desc: 'Create a verified company profile. Fast, free, and reviewed within 24 hours.', icon: <Building2 size={16} /> },
                  { step: '2', title: 'Post an opportunity', desc: 'Describe the role and required skills — live within minutes of approval.', icon: <FileText size={16} /> },
                  { step: '3', title: 'Review matched candidates', desc: 'See applications ranked by match score. Review profiles and resumes at a glance.', icon: <Zap size={16} /> },
                  { step: '4', title: 'Build your team', desc: 'Connect directly with motivated newcomer talent ready to contribute from day one.', icon: <TrendingUp size={16} /> },
                ].map((item) => (
                  <div key={item.step} className="flex gap-4 bg-white p-5 rounded-xl border border-gray-100 hover:border-gray-300 hover:shadow-sm transition-all">
                    <div className="w-9 h-9 bg-gray-900 text-white rounded-full flex items-center justify-center font-black text-sm flex-shrink-0">
                      {item.step}
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900 mb-0.5 text-sm">{item.title}</h4>
      ...
      <section className="bg-white py-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-gray-900">Newcomers who made it work</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-gray-50 p-7 rounded-2xl border border-gray-100">
              ...
              <p className="text-gray-700 leading-relaxed mb-6">
                &ldquo;Even having 10 years of experience as a Business Analyst, I was struggling to land a job in Ottawa.
                CanStart connected me with a local tech company for a micro-internship — now I have Canadian experience
                on my resume and a full-time offer on the table.&rdquo;
              </p>
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 bg-red-600 rounded-full flex items-center justify-center text-white font-bold">F</div>
                <div>
                  <p className="font-bold text-gray-900 text-sm">Fariha J.</p>
                  <p className="text-xs text-gray-500">Business Analyst · Ottawa, ON · From Bangladesh</p>
                </div>
              </div>
            </div>
            <div className="bg-gray-50 p-7 rounded-2xl border border-gray-100">
              ...
              <p className="text-gray-700 leading-relaxed mb-6">
                &ldquo;As a small business owner who immigrated from Ukraine, I understand the struggle. CanStart helped me
                find skilled volunteers who genuinely want to grow. It&apos;s a win-win — they get Canadian experience,
                I get real help growing my business.&rdquo;
              </p>
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 bg-gray-800 rounded-full flex items-center justify-center text-white font-bold">S</div>
                <div>
                  <p className="font-bold text-gray-900 text-sm">Svitlana H.</p>
                  <p className="text-xs text-gray-500">Entrepreneur · Toronto, ON · From Ukraine</p>
                </div>
              </div>
            </div>
          </div>
